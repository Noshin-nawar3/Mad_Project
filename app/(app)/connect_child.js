...
export default function ConnectChild() {
  ...
  const [connectedChildId, setConnectedChildId] = useState(null);
  const [quizResults, setQuizResults] = useState([]);
  const [raadsrResults, setRaadsrResults] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (connectedChildId) {
      fetchQuizResults(connectedChildId);
      fetchRaadsrResults(connectedChildId);
    }
  }, [connectedChildId]);
  ...
  const fetchRaadsrResults = async (childId) => {
    try {
      const q = query(
        collection(db, "raadsr_results"),
        where("userId", "==", childId) // Use userId instead of username for consistency
      );
      const querySnapshot = await getDocs(q);
      const results = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      })).reverse(); // Reverse to show latest first, matching loadHistory
      setRaadsrResults(results);
    } catch (error) {
      console.error("Error fetching Raadsr results:", error);
    }
  };
  ...
  const renderRaadsrResult = ({ item, index }) => {
    const historyIndex = raadsrResults.length - index;
    return (
      <View style={styles.card}>
        <Text style={styles.date}>#{historyIndex} | {item.date instanceof Date ? item.date.toLocaleString() : 
          item.date instanceof Timestamp ? item.date.toDate().toLocaleString() : 
          item.date || "N/A"}</Text>
        <Text style={styles.score}>Total Score: {item.score}</Text>
        <Text style={styles.sectionTitle}>Section Scores:</Text>
        {item.sectionScores && Object.entries(item.sectionScores).map(([sec, val]) => (
    ...
    <View style={styles.container}>
      {!connectedChildId ? (
        ...
        <>
          ...
          {/* Raadsr Test Results Section */}
          <View style={styles.raadsrContainer}>
            <Text style={styles.raadsrHeader}>Raadsr Test Results</Text>
            {raadsrResults.length > 0 ? (
              <FlatList
                data={raadsrResults}
                renderItem={renderRaadsrResult}
                keyExtractor={item => item.id}
                contentContainerStyle={styles.list}
              />
            ) : (
              <Text style={styles.noDataText}>No Raadsr test data available</Text>
            )}
          </View>
        </>
      )}
