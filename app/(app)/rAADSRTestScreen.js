...
// export default function rAADSRTestScreen() {
//   const [questions, setQuestions] = useState([]);
//   const [answers, setAnswers] = useState({});
//   const [loading, setLoading] = useState(true);
//   const { user } = useAuth();
...
//     const score = Object.values(answers).reduce((sum, v) => sum + (v || 0), 0); // Handle undefined values
//     const sections = calculateSectionScores(questions, answers);
//     const date = new Date().toISOString();
...
//       text: q.text,
//       category: q.category,
//       answer: answers[q.id] !== undefined ? answers[q.id] : null, // Default to null for unanswereds
//     }));
...
//     console.log('Result to save:', result); // Debug log to inspect data
//     try {
//       await addDoc(collection(db, 'raadsr_results'), result);
//       Alert.alert('Success', 'Test results saved successfully!');
//       router.push('/specialChild_dashboard');
//     } catch (error) {
//       console.error('Error saving result:', error);
//       Alert.alert('Error', 'Failed to save test results. Check console for details.');
//     }
//   };
...
export default function rAADSRTestScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        Alert.alert('Error', 'Failed to load draft. Starting new test.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePause = async () => {
    try {
      await AsyncStorage.setItem('@raadsr_draft', JSON.stringify({ answers, currentPage }));
      console.log('Draft saved:', { answers, currentPage });
      Alert.alert('Paused', 'Test paused. You can resume later from the dashboard.');
      router.push('/specialChild_dashboard');
    } catch (error) {
      console.error('Error saving draft:', error);
      Alert.alert('Error', 'Failed to save test progress.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const docRef = await addDoc(collection(db, 'raadsr_results'), result);
      console.log('Document written with ID:', docRef.id);
      await AsyncStorage.removeItem('@raadsr_draft');
      Alert.alert('Success', 'Test results saved successfully!');
      router.replace('/specialChild_dashboard');
    } catch (error) {
      console.error('Error saving result:', error);
      Alert.alert('Error', `Failed to save test results: ${error.message}`);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.title}>RAADS-R Test (Page {currentPage + 1} of {pages})</Text>
        {currentQuestions.map((q, index) => (
          <View key={q.id} style={styles.questionContainer}>
            <Text style={styles.questionText}>
              {currentPage * pageSize + index + 1}. {q.text}
            </Text>
            {OPTIONS.map(opt => (
              <TouchableOpacity
                key={opt.label}
      ...
      <View style={styles.footer}>
        <View style={styles.navRow}>
          ...
          <TouchableOpacity
            ...
            onPress={handlePause}
          >
            <Text style={styles.navText}>Pause</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.navButton, currentPage === pages - 1 && styles.disabledButton]}
            onPress={() => setCurrentPage(p => Math.min(p + 1, pages - 1))}
