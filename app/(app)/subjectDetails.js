...
//       const courseEnrolledRef = ref(db, `users/${user.uid}/courseEnrolled`);
//       const newCourseRef = push(courseEnrolledRef);
...
//       const db = getDatabase();
//       const courseEnrolledRef = ref(db, `users/${user.uid}/courseEnrolled`);
//       const snapshot = await get(courseEnrolledRef);
//       if (snapshot.exists()) {
...
//         );
//         if (courseKey) {
//           await remove(ref(db, `users/${user.uid}/courseEnrolled/${courseKey}`));
...
import { collection, addDoc, serverTimestamp, doc, getDocs, query, where, deleteDoc } from "firebase/firestore";
...
export default function SubjectDetails() {
  ...
  // Fetch enrolled levels on component mount
  useEffect(() => {
    const fetchEnrolledLevels = async () => {
      try {
        const auth = getAuth();
        const user = auth.currentUser;
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          enrolled[`Level ${data.level}`] = true; // Match the level format (e.g., "Level 1")
        });
        setEnrolledLevels(enrolled);
      } catch (error) {
        console.error("Error fetching enrolled levels:", error);
        Alert.alert("Error", "Failed to load enrollment status.");
      }
    };

    fetchEnrolledLevels();
  }, [subject]); // Re-run if subject changes
  ...
  const handleEnroll = async (lvl) => {
    try {
      ...
      const courseEnrolledCollection = collection(db, "courseEnrolled");
      await addDoc(courseEnrolledCollection, {
        userId: user.uid,
        subject,
        level: parseInt(lvl.split(" ")[1]), // Extract numeric level (e.g., 1 from "Level 1")
        title: `${subject} ${lvl}`,
        description: courseDetails.description,
        image: courseDetails.image,
        length: courseDetails.length,
      ...
      console.log(`Enrolled in ${subject} ${lvl}`);
      Alert.alert("Success", `Enrolled in ${subject} ${lvl} successfully!`);
    } catch (error) {
      console.error(`Error enrolling in ${lvl}:`, error);
      Alert.alert("Error", `Failed to enroll in ${lvl}: ${error.message}`);
      setEnrolledLevels((prev) => ({ ...prev, [lvl]: false }));
    }
  };
  ...
  const handleUnenroll = async (lvl) => {
    try {
      ...
      const q = query(
        ...
        where("userId", "==", user.uid),
        where("subject", "==", subject),
        where("level", "==", parseInt(lvl.split(" ")[1]))
      );
      const querySnapshot = await getDocs(q);

      if (!querySnapshot.empty) {
        const docToDelete = querySnapshot.docs[0];
        await deleteDoc(doc(db, "courseEnrolled", docToDelete.id));
        console.log(`Unenrolled from ${subject} ${lvl}`);
        Alert.alert("Success", `Unenrolled from ${subject} ${lvl} successfully!`);
      }
    } catch (error) {
      console.error(`Error unenrolling from ${lvl}:`, error);
      Alert.alert("Error", `Failed to unenroll from ${lvl}: ${error.message}`);
      setEnrolledLevels((prev) => ({ ...prev, [lvl]: true }));
    }
  };
